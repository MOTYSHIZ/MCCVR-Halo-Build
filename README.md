...
5. Start your headset connection and OpenXR runtime. SteamVR is the currently
   recommended/tested route; make it the active OpenXR runtime.
6. With MCC closed, run `HaloMCCVRLauncher.exe` from `Halo_MCC_VR` and use the
...
7. Set the required 120° FOV in **every supported game's own settings**, then
...
1. Close MCC completely and back up the existing `Halo_MCC_VR` folder.
2. Replace `HaloMCCVR.dll` and `HaloMCCVRLauncher.exe` with the new pair.
3. **Keep your existing `halomccvr.cfg`** so your controls, alignment, seat, and
...
4. If you use both Steam and Store editions, update each edition's separate
   `Halo_MCC_VR` folder.
5. Remove obsolete `halo3xr.dll` / `halo3xr_launcher.exe` from the active mod
