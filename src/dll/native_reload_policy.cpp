...
template<unsigned I> void __fastcall StateHook(uint32_t weapon,int16_t magazine,int32_t state)
{
    ...
    __try {
        ...
        Shorten(I,weapon,magazine);
    } __finally { timingScope=previous;r.callbacks.fetch_sub(1,std::memory_order_release); }
}
template<unsigned I> void __fastcall ActionHook(uint32_t unit,uint32_t weapon,uint32_t action,int32_t slot,uint8_t flag)
{
    ...
    } __finally { animationScope=previous;timingScope=previousTiming;r.callbacks.fetch_sub(1,std::memory_order_release); }
}
template<unsigned I> uint8_t __fastcall PlayHook(uint32_t user,uint32_t slot,uint32_t animation,uint8_t flag)
{
    ...
    __try {
        ...
        else if constexpr(I==native_reload::H3 || I==native_reload::ODST || I==native_reload::H4)
...
void NativeReloadPolicy_Poll()
{
    ...
    for (unsigned i=0;i<native_reload::Count;++i) {
        ...
        const unsigned requested=g_config.manual_reload ?
            (g_config.manual_reload_disable_auto?1u:0u)|(g_config.manual_reload_skip_animations?2u:0u) : 0;
        if (!r.attempted && requested) {
            ...
            LOG("Native reload policy %s: disable-auto=%s skip-animation=%s; native ammo transfer, local owner only; headset verification pending",
        ...
        r.options.store(requested & ((r.autoReady?1u:0u)|(r.skipReady?2u:0u)),std::memory_order_release);
