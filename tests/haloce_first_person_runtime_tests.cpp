...
static bool contextValid=true,nativeSawInvalidated=true;
...
int main()
{
    ...
    CHECK(quiescenceCalls==2&&quiescenceRanges==9);
    ...
    CHECK(quiescenceCalls==2&&quiescenceRanges==9);
    std::puts("PASS production CE first-person receipt: scale/projection, failed prepare, renderer/recenter/generation/tracking, recovery and nine-hook retirement");
    return 0;
}
